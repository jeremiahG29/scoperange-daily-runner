...
The minimum production evidence runner is now represented by a disabled bridge and an inactive candidate at `inactive-production-daily-workflow.yml`. The candidate remains outside `.github/workflows/**`; its checked-in bridge stops before environment or secret access and therefore performs no checkout, public request, database connection, or evidence write. The private execution package is exact-commit-bound and evidence-only, while proposal review, promotion, rollback, live pricing, workflow registration, and activation remain unavailable.
...
The public package includes an inactive production workflow candidate and bridge for the credentialless fetch, durable lease, and evidence-only writer implementation bound by `production-source-lock.example.json`. The candidate is deliberately outside `.github/workflows/**`, its checked-in gate remains false, and its runner job cannot become eligible. No source target, credential, database connection, evidence ingestion, proposal, promotion, or pricing authority is configured or activated.
