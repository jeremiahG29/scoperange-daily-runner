...
This package is a public-safe automation control plane. It contains no ScopeMatch application source, estimation logic, evidence, production target, credential, or pricing authority. The reusable inert shell remains registered and provider-disabled. The production schedule is registered at `.github/workflows/scoperange-daily.yml`; its bridge is implemented, but the external exact bindings and release state remain unconfigured, and the workflow remains disabled at the provider.

The registered production workflow is byte-for-byte identical to `inactive-production-daily-workflow.yml`. Its non-secret gate rejects before secret access unless an external post-merge control supplies the exact public commit, private commit, source-lock digest, and authorized release state. Those repository variables are intentionally absent in this source gate, so no private checkout, public request, database connection, or evidence write is enabled. The private execution package remains exact-commit-bound and evidence-only, while proposal review, promotion, rollback, and live pricing remain unavailable.

The source lock identifies one immutable private commit and a deterministic seven-file bundle without copying those files. A separately authorized activation may use the already provisioned repository-scoped read-only deploy key, strict GitHub SSH host verification, an exact-commit fetch, source verification, and ephemeral cleanup. Provider disablement and absent release bindings prevent those operations in the current state.
...
The production workflow consumes an external post-merge binding for the exact approved public commit. The binding remains deliberately unconfigured: no commit is embedded into the commit that must approve itself, and no default-branch fallback is allowed. Dependency caching is explicitly disabled.

The exact-locked private bundle includes the durable lease, missed-run, bounded replay, cancellation, and resume adapter. It remains inactive while the public workflow is provider-disabled and the external release bindings are absent.
...
Any future governance binding, private-source credential use, provider-level egress restriction, workflow enablement, temporary database login window, proof run, recurrence, or acquisition step requires its own review and explicit authorization. None is authorized by this source package.

## Minimum production runner activation closure

The public package includes a registered, provider-disabled production workflow and a complete bridge for the credentialless fetch, durable lease, and evidence-only writer implementation bound by `production-source-lock.example.json`. The registered file is identical to the retained candidate. The gate writes its single authorization output only after exact external public-commit, private-commit, source-lock, and release-state checks pass, then the runner rechecks those bindings before reading secrets.

The initial plan is limited to one public recheck of the previously recorded `src-atlanta-door-refinishing-blog` source for `coverage:painting:exterior-door:v1`. It retains only provenance and content fingerprints; numeric evidence, proposals, promotion, rollback, and live pricing remain forbidden. The workflow also exposes an inputless manual recovery trigger for a separately authorized controlled run when the scheduled window cannot be attended. That path is bound to the GitHub run identifier, uses its dispatch time as the recurrence boundary, and remains inert unless the same exact external bindings, provider enablement, protected-environment approval, and bounded database login are supplied. The repository variables are not configured by this source change, the workflow remains provider-disabled, and the production database role remains outside this repository's authority.
